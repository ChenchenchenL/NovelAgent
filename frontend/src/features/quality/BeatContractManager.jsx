...
export function BeatContractManager({ currentSceneId }) {
  ...
  if (!currentSceneId) return <div className="empty-state">请先在左侧选择场景以管理 Beat 契约</div>

  return (
    <div className="continuity-subpanel">
      <form onSubmit={handleCreateBeat} style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
        ...
        <input
          ...
          style={{ width: '80px' }}
        />
        <button type="submit" className="btn-primary">➕ 创建 Beat</button>
      </form>

      <div className="continuity-list">
        {beats.length === 0 && <div className="empty-state">当前场景尚未创建 Beat 契约约束</div>}
        {beats.map((b) => (
          <div key={b.id} className="continuity-card">
            <div className="continuity-card-header">
              <span><strong>Beat #{b.id}</strong> (目标: {b.target_word_count || 0} 字)</span>
              <span className={`badge ${b.status === 'COMPLETED' ? 'success' : (b.status === 'OVERRUN' ? 'warning' : 'gray')}`}>
                {b.status}
              </span>
            </div>
            <div style={{ marginTop: '8px', fontSize: '13px' }}>
              <div><strong>要求推进:</strong> {b.required_advancements.map((r, i) => <span key={i} className="badge gray">[{r.type}] {r.description}</span>)}</div>
            ...
            {activeAdvanceBeatId === b.id ? (
              <div style={{ marginTop: '10px', display: 'flex', gap: '6px' }}>
                <input
                  type="text"
                  placeholder="输入达成的推进描述..."
                  value={advanceInput}
                  onChange={(e) => setAdvanceInput(e.target.value)}
                  style={{ flex: 1 }}
                />
                <button className="btn-small btn-primary" onClick={() => handleAdvanceSubmit(b.id)}>提交推进</button>
